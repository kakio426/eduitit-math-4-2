...
제공된 비상교육 4-2 교사용 교과서 PDF의 Unit 3 `소수의 덧셈과 뺄셈` 자료를 학습 근거로 사용했습니다. PDF 128–129쪽 ↔ 교과서 70–71쪽에서 소수 한 자리 수 덧셈, 수직선, 소수점 맞추기를 확인했습니다. PDF 130–131쪽 ↔ 수학 익힘 44–45쪽에서 소수점 정렬 오류를 확인했습니다. PDF 132–133쪽 ↔ 교과서 72–73쪽에서 백분 격자와 소수 두 자리 수의 받아올림을, PDF 134–135쪽 ↔ 수학 익힘 46–47쪽에서 덧셈 연습과 합의 비교를 확인했습니다. 교과서 이미지는 학생 실행 자산으로 사용하지 않았습니다.

## 구현

목표는 `소수 두 자리 수의 덧셈을 계산하기`, 성취기준은 `[4수01-16]`입니다. 10개 문항은 십분 격자, 수직선 이동, 소수점 정렬, 백분 격자, 받아올림, 자릿수가 다른 소수, 합이 10을 넘는 계산, 잘못된 계산 찾기, 두 합 비교, 계산 방법 설명을 구조화된 HTML과 데이터로 제공합니다. 격자·수직선·세로셈은 HTML/CSS로 표현하며 정답 판정은 JS 데이터가 담당합니다. 생성 그림은 분위기와 진행을 돕는 context-only 자산이며 정답의 유일한 근거가 아닙니다.

## 자산·라인리지

17개 신규 자산(표지·제목·튜토리얼 2·보상 7·결과 6)을 `mathmon-grade4-official-pack-v1`의 `mathmon-grade4-01-sugar-glider` 하늘주머니몬 기준 이미지와 함께 실제 순차 imagegen 호출로 생성했습니다. 원본은 `_lessons/4-2-3-3-mathmon-decimal-add-charge/assets/*-source.png`, runtime은 1280×800 `*-generated.webp`입니다. 생성 ID·프롬프트 SHA-256·원본/runtime SHA-256·허용 후처리 계보는 `_lessons/4-2-3-3-mathmon-decimal-add-charge/assets/generation-lineage.json`에 기록했습니다. 전역 provider registry는 변경하지 않았으며, 현재 라인리지는 `honest-nonstrict-provider-lineage`만 주장합니다.

보상 7종은 `reward-states-contact-sheet.png`, 결과 6종은 `result-states-contact-sheet.png`에서 함께 확인할 수 있습니다. 결과 6장은 오른쪽 패널 위쪽에 각 단계 이름을 생성 래스터로 직접 넣고, 아래쪽 동적 정답 수·다음 목표 영역을 비운 서로 다른 충전소 장면입니다.

## 튜토리얼

튜토리얼은 정확히 2장입니다. 첫 포스터는 `소수점을 맞추고 같은 자리끼리 더해요`와 `0.73 + 0.24 = 0.97`로 계산 방법을 보여 줍니다. 둘째 포스터는 `10문제를 풀면 충전소가 6단계로 밝아져요`와 `마지막에 하늘주머니몬을 만나요`로 보상 목표를 보여 줍니다. 두 포스터의 한국어 안내문과 이동 버튼은 생성 래스터 자체에 포함했고, HTML/CSS 글자 덮어쓰기를 사용하지 않았습니다.

## 학생 문구 검토

Humanizer 기준으로 학생 문구를 묶어 검토했습니다. `소수점을 맞추고 같은 자리끼리 더해요`, `합이 10 이상이면 받아올려요`, `충전소 불빛이 하나 켜졌어요`처럼 4학년 학생이 소리 내어 읽어도 바로 이해할 수 있는 말로 확정했습니다. `2.53+0.69`의 두 번 받아올림도 실제 계산 순서대로 설명합니다.

## 검증 결과

- `node scripts/test-mathmon-grade4-decimal-add-charge-phase-a.mjs`: PASS
- `node scripts/build-lesson.mjs 4-2-3-3-mathmon-decimal-add-charge`: PASS
- `node scripts/check-lesson-contract.mjs 4-2-3-3-mathmon-decimal-add-charge`: PASS
- `node scripts/check-lesson-visual-contract.mjs 4-2-3-3-mathmon-decimal-add-charge`: PASS
- `MATHMON_QA_SEED=20260831 node scripts/qa-lesson-flow.mjs 4-2-3-3-mathmon-decimal-add-charge 20260831`: PASS — 3개 viewport, 전체 흐름 PNG 51장
- 표지·튜토리얼 2장·문제·보상·결과 화면의 직접 시각 검토: PASS
- 현재 실행본·정책·하네스 지문과 보고서 증거는 `screenshots/report-evidence-manifest.json`에 동기화합니다.
...
- 실행본 SHA-256: `3909bd154e0222e93a3e7aa302b4e9436a3b9b554a2eb39e977e89eec232c5d8`
- 생성 시각: `2026-08-31T05:53:49.489Z`
- 등록 회귀 이름: `3개`
- 실제 실행 화면 조건: `3개`
- 동일 조건 별칭 통합: `0개`
...
- 학생이 보는 것: 소수 덧셈 충전소 제목과 한 줄 목표, 시작 버튼을 봅니다.
- 판단하거나 누르는 것: 게임을 시작할 준비가 되면 시작을 누릅니다.
- 화면에서 확인되는 수학 관계: 소수 두 자리 수의 덧셈을 배우는 차시임을 확인합니다.
...
- 학생이 보는 것: 소수 두 자리 수의 덧셈 문제를 푸는 방법과 게임 흐름을 그림으로 봅니다.
- 판단하거나 누르는 것: 그림 속 순서와 누를 곳을 확인한 뒤 다음 행동 버튼을 누릅니다.
- 화면에서 확인되는 수학 관계: 소수 두 자리 수의 덧셈에서 무엇을 비교하거나 계산하는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 소수 두 자리 수의 덧셈을 이용해 선택지를 판단합니다.
...
- 화면에서 확인되는 수학 관계: 소수 두 자리 수의 덧셈의 관계와 고른 답이 왜 맞지 않는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 소수 두 자리 수의 덧셈의 완성값을 보상 화면 전에 다시 확인합니다.
...
- 판단하거나 누르는 것: 이번 충전소 완성 단계 변화를 확인하기 위해 열기를 누릅니다.
...
- 학생이 보는 것: 보상 사건 그림과 이번 충전소 완성 단계 변화, 다음 행동 버튼을 봅니다.
...
- 화면에서 확인되는 수학 관계: 한 판의 정답과 충전소 완성 단계 변화가 하나의 결과 단계로 정리됩니다.
- 다음 상태로 넘어가는 이유: 다시를 누르면 새 문제 순서와 새 보상 흐름으로 시작합니다.

#### 결과 단계 · first-charge · `engine-flow-desktop-08a-result-first-charge.png`
...
#### 결과 단계 · spark-charge · `engine-flow-desktop-08a-result-spark-charge.png`
...
#### 결과 단계 · half-charge · `engine-flow-desktop-08a-result-half-charge.png`
...
#### 결과 단계 · bright-charge · `engine-flow-desktop-08a-result-bright-charge.png`
...
#### 결과 단계 · full-charge · `engine-flow-desktop-08a-result-full-charge.png`
...
#### 결과 단계 · rainbow-charge · `engine-flow-desktop-08a-result-rainbow-charge.png`
...
#### 결과 단계 · first-charge · `engine-flow-tablet-landscape-08a-result-first-charge.png`
...
#### 결과 단계 · spark-charge · `engine-flow-tablet-landscape-08a-result-spark-charge.png`
...
#### 결과 단계 · half-charge · `engine-flow-tablet-landscape-08a-result-half-charge.png`
...
#### 결과 단계 · bright-charge · `engine-flow-tablet-landscape-08a-result-bright-charge.png`
...
#### 결과 단계 · full-charge · `engine-flow-tablet-landscape-08a-result-full-charge.png`
...
#### 결과 단계 · rainbow-charge · `engine-flow-tablet-landscape-08a-result-rainbow-charge.png`
...
#### 결과 단계 · first-charge · `engine-flow-reported-design-feedback-1079x842-08a-result-first-charge.png`
...
#### 결과 단계 · spark-charge · `engine-flow-reported-design-feedback-1079x842-08a-result-spark-charge.png`
...
#### 결과 단계 · half-charge · `engine-flow-reported-design-feedback-1079x842-08a-result-half-charge.png`
...
#### 결과 단계 · bright-charge · `engine-flow-reported-design-feedback-1079x842-08a-result-bright-charge.png`
...
#### 결과 단계 · full-charge · `engine-flow-reported-design-feedback-1079x842-08a-result-full-charge.png`
...
#### 결과 단계 · rainbow-charge · `engine-flow-reported-design-feedback-1079x842-08a-result-rainbow-charge.png`
