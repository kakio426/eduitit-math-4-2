...
- 차시: `4-2-2-3-mathmon-quadrilateral-workshop`
- 목표: 여러 사각형의 성질을 살펴보고 분류하기
- 성취기준: `[4수03-10]`
- 캐릭터: `mathmon-grade4-07-narwhal` / 별뿔고래몬
- 자산 팩: `mathmon-grade4-official-pack-v1`
- 주 근거: 제공 교과서 PDF 86–87쪽 ⇔ 인쇄 44–45쪽. 사다리꼴·평행사변형·마름모·직사각형·정사각형의 평행한 변, 변의 길이, 각의 크기 성질과 여러 이름으로 분류되는 관계를 반영했다.

## 실제 생성 자산

`1 cover + 1 title + 2 tutorial + 7 reward + 6 result = 17종`을 순차 imagegen으로 생성했다. 실제 generation ID, 원본/runtime 경로와 SHA-256, 후처리 기록은 [`assets/generation-lineage.json`](assets/generation-lineage.json)에 있다. 결과 컨택시트는 [`assets/result-states-contact-sheet.png`](assets/result-states-contact-sheet.png), 보상 컨택시트는 [`assets/reward-states-contact-sheet.png`](assets/reward-states-contact-sheet.png)다. 결과 6장은 플레이 진행 슬롯에서 같은 장면을 의도적으로 재사용하며 별도 플레이 세트를 주장하지 않는다.

## 수정 이력

초기 튜토리얼 포스터에서 큰 빈 패널 때문에 설명 문구가 보이지 않던 결함을 다시 확인했다. P1/P2를 이미지 자체에 큰 고대비 한국어 안내가 들어간 새 실제 생성 래스터로 교체했으며, 이전 출력은 `assets/generation-lineage.json`의 `superseded-nonruntime` 기록으로 남겼다. 교체 후 최종 PNG와 브라우저 캡처에서 문구를 직접 확인했다.

## 학습·문구 검토

10문항은 사각형 공통 조건, 사다리꼴, 평행사변형, 마름모, 직사각형, 정사각형, 성질 표, 회전, 여러 이름, 설명하기로 구성했다. 생성 이미지는 context-only이고 정답은 구조화된 HTML 데이터다. 학생 문구는 Humanizer 기준으로 검토했다. 짧은 문장과 익숙한 낱말을 우선했고, 어색한 번역투를 고쳤으며 자연도 A, 의미 보존 6항 모두 통과다.

## 화면과 QA

흐름은 `cover → tutorial-1 → tutorial-2 → play → reward closed/revealed → result`다. stage-reveal 보상과 6단계 결과를 사용한다. QA viewport는 `1280×800`, `1024×768`, `1079×842`다.
...
- browser QA: PASS (3 viewport, 51 screenshots; closed/revealed reward and six result tiers)
- evidence sync/report-current-screen-evidence-v3: PASS
- legacy delivery verifier: PASS (`MATHMON_DELIVERY_GATE: PASS`, 1 lesson, 158 changed paths, route `full-qa`)

## 정직한 provenance 상태

`generation-lineage.json`은 모델명이 도구 결과에 노출되지 않아 추정하지 않은 honest non-strict provider lineage다. 독립 provider promotion이 없어 strict final approval은 blocked이며, 이를 PASS로 표시하지 않는다.

## 화면 크기

브라우저 증거는 위 세 viewport에서 다시 촬영하고 `screenshots/mathmon-browser-qa-receipt.json` 및 `report-evidence-manifest.json`에 현재 런타임·정책·하네스 지문과 함께 기록한다.

<!-- REPORT-EVIDENCE-ALL:START -->

## 2026-08-31 최신 원본 스크린샷 전수

- 실행본 SHA-256: `d997824993e3bef61cb2b47b8e3c60b813e2c775516fc1ab14dacd2803ed54e7`
- 생성 시각: `2026-08-31T05:52:28.077Z`
- 등록 회귀 이름: `3개`
- 실제 실행 화면 조건: `3개`
- 동일 조건 별칭 통합: `0개`
...
- 학생이 보는 것: 사각형 정비소 제목과 한 줄 목표, 시작 버튼을 봅니다.
- 판단하거나 누르는 것: 게임을 시작할 준비가 되면 시작을 누릅니다.
- 화면에서 확인되는 수학 관계: 여러 가지 사각형을 배우는 차시임을 확인합니다.
...
- 학생이 보는 것: 여러 가지 사각형 문제를 푸는 방법과 게임 흐름을 그림으로 봅니다.
- 판단하거나 누르는 것: 그림 속 순서와 누를 곳을 확인한 뒤 다음 행동 버튼을 누릅니다.
- 화면에서 확인되는 수학 관계: 여러 가지 사각형에서 무엇을 비교하거나 계산하는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 여러 가지 사각형을 이용해 선택지를 판단합니다.
...
- 화면에서 확인되는 수학 관계: 여러 가지 사각형의 관계와 고른 답이 왜 맞지 않는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 여러 가지 사각형의 완성값을 보상 화면 전에 다시 확인합니다.
...
- 판단하거나 누르는 것: 이번 정비소 단계 변화를 확인하기 위해 열기를 누릅니다.
...
- 학생이 보는 것: 보상 사건 그림과 이번 정비소 단계 변화, 다음 행동 버튼을 봅니다.
...
- 화면에서 확인되는 수학 관계: 한 판의 정답과 정비소 단계 변화가 하나의 결과 단계로 정리됩니다.
- 다음 상태로 넘어가는 이유: 다시를 누르면 새 문제 순서와 새 보상 흐름으로 시작합니다.

#### 결과 단계 · first-workshop · `engine-flow-desktop-08a-result-first-workshop.png`
...
#### 결과 단계 · second-workshop · `engine-flow-desktop-08a-result-second-workshop.png`
...
#### 결과 단계 · third-workshop · `engine-flow-desktop-08a-result-third-workshop.png`
...
#### 결과 단계 · fourth-workshop · `engine-flow-desktop-08a-result-fourth-workshop.png`
...
#### 결과 단계 · fifth-workshop · `engine-flow-desktop-08a-result-fifth-workshop.png`
...
#### 결과 단계 · sixth-workshop · `engine-flow-desktop-08a-result-sixth-workshop.png`
...
#### 결과 단계 · first-workshop · `engine-flow-tablet-landscape-08a-result-first-workshop.png`
...
#### 결과 단계 · second-workshop · `engine-flow-tablet-landscape-08a-result-second-workshop.png`
...
#### 결과 단계 · third-workshop · `engine-flow-tablet-landscape-08a-result-third-workshop.png`
...
#### 결과 단계 · fourth-workshop · `engine-flow-tablet-landscape-08a-result-fourth-workshop.png`
...
#### 결과 단계 · fifth-workshop · `engine-flow-tablet-landscape-08a-result-fifth-workshop.png`
...
#### 결과 단계 · sixth-workshop · `engine-flow-tablet-landscape-08a-result-sixth-workshop.png`
...
#### 결과 단계 · first-workshop · `engine-flow-reported-design-feedback-1079x842-08a-result-first-workshop.png`
...
#### 결과 단계 · second-workshop · `engine-flow-reported-design-feedback-1079x842-08a-result-second-workshop.png`
...
#### 결과 단계 · third-workshop · `engine-flow-reported-design-feedback-1079x842-08a-result-third-workshop.png`
...
#### 결과 단계 · fourth-workshop · `engine-flow-reported-design-feedback-1079x842-08a-result-fourth-workshop.png`
...
#### 결과 단계 · fifth-workshop · `engine-flow-reported-design-feedback-1079x842-08a-result-fifth-workshop.png`
...
#### 결과 단계 · sixth-workshop · `engine-flow-reported-design-feedback-1079x842-08a-result-sixth-workshop.png`
