...
제공된 비상교육 4-2 교사용 교과서 PDF의 5단원 `꺾은선그래프` 자료를 학습 근거로 사용했습니다. PDF 210–211쪽 ↔ 교과서 118–119쪽에서 오른쪽 위로 올라가는 선분은 증가, 오른쪽 아래로 내려가는 선분은 감소, 수평인 선분은 변화 없음을 나타내며 선분이 많이 기울수록 변화가 큼을 확인했습니다. 같은 범위에서 가장 많이 늘어난 때, 늘고 줄어드는 전체 순서, 값이 같은 구간, 그래프에서 찾은 사실을 바탕으로 한 조심스러운 예상을 살폈습니다. PDF 212–213쪽 ↔ 수학 익힘 70–71쪽에서는 가장 큰 증가·감소, 두 시점의 차, 그래프에서 추가로 알 수 있는 내용을 설명하는 활동을 확인했습니다. 교과서 이미지는 학생 실행 자산으로 사용하지 않았습니다.

## 구현

목표는 `꺾은선그래프에서 증가와 감소의 변화를 설명하기`, 성취기준은 `[4수04-03]`입니다. 10개 문항은 증가, 감소, 변화 없음, 두 시점의 변화량, 가장 큰 증가, 가장 큰 감소, 늘다가 줄어드는 전체 변화, 값이 같은 구간, 그래프에서 바로 확인할 수 있는 사실, 근거를 함께 밝힌 조심스러운 예상을 차례로 다룹니다. 그래프는 수치 데이터에서 만든 SVG로 표현하고 정답 판정은 구조화된 JS 데이터가 담당합니다. 생성 그림은 탐정 연구소의 진행과 보상 분위기를 돕는 context-only 자산이며 정답의 유일한 근거가 아닙니다.

## 자산·라인리지

17개 신규 자산(표지·제목·튜토리얼 2·보상 7·결과 6)을 `mathmon-grade4-official-pack-v1`의 `mathmon-grade4-09-giant-anteater` 긴꼬리개미핥기몬 기준 이미지와 함께 실제 순차 imagegen 호출로 생성했습니다. 원본은 `_lessons/4-2-5-3-mathmon-trend-detective/assets/*-source.png`, runtime은 1280×800 `*-generated.webp`입니다. 실제 generation ID·프롬프트 SHA-256·원본/runtime SHA-256·참조 입력·허용 후처리 계보는 `_lessons/4-2-5-3-mathmon-trend-detective/assets/generation-lineage.json`에 기록했습니다. 픽셀 합성이나 HTML/CSS 글자 덮어쓰기는 사용하지 않았습니다. 전역 provider registry는 변경하지 않았고 현재 라인리지는 `honest-nonstrict-provider-lineage`만 주장합니다.

보상 7종은 `reward-states-contact-sheet.png`, 결과 6종은 `result-states-contact-sheet.png`에서 함께 확인할 수 있습니다. 결과 6장은 오른쪽 패널 위쪽에 `첫 단서`, `방향 화살표`, `변화 기록장`, `단서 추적판`, `탐정 연구소`, `별빛 탐정 연구소`를 생성 래스터로 직접 넣고, 아래쪽 동적 정답 수·다음 목표 영역을 비운 서로 다른 연구소 완성 장면입니다. 최종 1280×800 PNG에서 패널 중심을 단계별로 픽셀 측정해 계약과 lineage에 기록했습니다.

## 튜토리얼

튜토리얼은 정확히 2장입니다. 첫 포스터는 `오른쪽 위로 가면 증가예요`, `오른쪽 아래로 가면 감소예요`, `수평이면 변화가 없어요`로 선분 방향을 읽는 법을 보여 줍니다. 둘째 포스터는 `두 시점의 값 차이를 비교해요`, `가장 많이 늘거나 줄어든 구간을 찾아요`, `그래프에서 확인한 사실을 근거로 말해요`, `10문제를 풀면 긴꼬리개미핥기몬의 연구소가 6단계로 완성돼요`로 변화 비교와 보상 목표를 보여 줍니다. 두 포스터의 한국어 안내문과 이동 버튼은 생성 래스터 자체에 포함했고, HTML/CSS 글자 덮어쓰기를 사용하지 않았습니다.

## 학생 문구 검토

Humanizer 기준으로 학생 문구를 묶어 검토했습니다. `선분이 오른쪽 위로 올라가요`, `두 시점의 값을 읽고 차이를 구해요`, `그래프에 실제로 표시된 값과 선분만 근거로 판단해요`처럼 4학년 학생이 소리 내어 읽어도 바로 이해할 수 있는 말로 확정했습니다. 미래 값은 `반드시`라고 단정하지 않고 `가능성이 있지만 아직 확정할 수 없어요`라고 구분했습니다. 기울기나 변화율 공식을 덧붙이지 않았습니다.

## 검증 결과

- `node scripts/test-mathmon-grade4-trend-detective-phase-a.mjs`: PASS
- `node scripts/build-lesson.mjs 4-2-5-3-mathmon-trend-detective`: PASS
...
- 실행본 SHA-256: `7ce6a3876060948b8826076c66aee1d7614876589252d48995cf4224e9e28b65`
- 생성 시각: `2026-08-31T05:56:28.459Z`
- 등록 회귀 이름: `3개`
- 실제 실행 화면 조건: `3개`
- 동일 조건 별칭 통합: `0개`
...
- 학생이 보는 것: 변화 추적 탐정 제목과 한 줄 목표, 시작 버튼을 봅니다.
- 판단하거나 누르는 것: 게임을 시작할 준비가 되면 시작을 누릅니다.
- 화면에서 확인되는 수학 관계: 꺾은선그래프의 증가와 감소을 배우는 차시임을 확인합니다.
...
- 학생이 보는 것: 꺾은선그래프의 증가와 감소 문제를 푸는 방법과 게임 흐름을 그림으로 봅니다.
- 판단하거나 누르는 것: 그림 속 순서와 누를 곳을 확인한 뒤 다음 행동 버튼을 누릅니다.
- 화면에서 확인되는 수학 관계: 꺾은선그래프의 증가와 감소에서 무엇을 비교하거나 계산하는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 꺾은선그래프의 증가와 감소을 이용해 선택지를 판단합니다.
...
- 화면에서 확인되는 수학 관계: 꺾은선그래프의 증가와 감소의 관계와 고른 답이 왜 맞지 않는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 꺾은선그래프의 증가와 감소의 완성값을 보상 화면 전에 다시 확인합니다.
...
- 판단하거나 누르는 것: 이번 탐정 단계 변화를 확인하기 위해 열기를 누릅니다.
...
- 학생이 보는 것: 보상 사건 그림과 이번 탐정 단계 변화, 다음 행동 버튼을 봅니다.
...
- 화면에서 확인되는 수학 관계: 한 판의 정답과 탐정 단계 변화가 하나의 결과 단계로 정리됩니다.
- 다음 상태로 넘어가는 이유: 다시를 누르면 새 문제 순서와 새 보상 흐름으로 시작합니다.

#### 결과 단계 · first-clue · `engine-flow-desktop-08a-result-first-clue.png`
...
#### 결과 단계 · direction-arrow · `engine-flow-desktop-08a-result-direction-arrow.png`
...
#### 결과 단계 · change-note · `engine-flow-desktop-08a-result-change-note.png`
...
#### 결과 단계 · clue-board · `engine-flow-desktop-08a-result-clue-board.png`
...
#### 결과 단계 · detective-lab · `engine-flow-desktop-08a-result-detective-lab.png`
...
#### 결과 단계 · starlight-detective-lab · `engine-flow-desktop-08a-result-starlight-detective-lab.png`
...
#### 결과 단계 · first-clue · `engine-flow-tablet-landscape-08a-result-first-clue.png`
...
#### 결과 단계 · direction-arrow · `engine-flow-tablet-landscape-08a-result-direction-arrow.png`
...
#### 결과 단계 · change-note · `engine-flow-tablet-landscape-08a-result-change-note.png`
...
#### 결과 단계 · clue-board · `engine-flow-tablet-landscape-08a-result-clue-board.png`
...
#### 결과 단계 · detective-lab · `engine-flow-tablet-landscape-08a-result-detective-lab.png`
...
#### 결과 단계 · starlight-detective-lab · `engine-flow-tablet-landscape-08a-result-starlight-detective-lab.png`
...
#### 결과 단계 · first-clue · `engine-flow-reported-design-feedback-1079x842-08a-result-first-clue.png`
...
#### 결과 단계 · direction-arrow · `engine-flow-reported-design-feedback-1079x842-08a-result-direction-arrow.png`
...
#### 결과 단계 · change-note · `engine-flow-reported-design-feedback-1079x842-08a-result-change-note.png`
...
#### 결과 단계 · clue-board · `engine-flow-reported-design-feedback-1079x842-08a-result-clue-board.png`
...
#### 결과 단계 · detective-lab · `engine-flow-reported-design-feedback-1079x842-08a-result-detective-lab.png`
...
#### 결과 단계 · starlight-detective-lab · `engine-flow-reported-design-feedback-1079x842-08a-result-starlight-detective-lab.png`
