...
제공된 비상교육 4-2 교사용 교과서 PDF의 6단원 `평면도형의 이동` 가운데 점 이동 자료를 학습 근거로 사용했습니다. PDF 236–239쪽 ↔ 교과서 132–135쪽에서 점을 위쪽·아래쪽·왼쪽·오른쪽으로 주어진 칸 수만큼 이동하고, 이동 전후 위치를 방향과 거리로 설명하는 흐름을 확인했습니다. 두 방향으로 이어서 움직일 때는 첫 이동의 도착점에서 다음 이동을 시작하며, 같은 가로·세로 이동을 모두 마치면 순서를 바꾸어도 같은 곳에 도착할 수 있음을 살폈습니다. PDF 240–241쪽 ↔ 수학 익힘 78–79쪽에서는 이동 전 점을 거꾸로 찾고 두세 번의 이동 명령으로 도착점을 찾는 문제를 확인했습니다. 교과서 이미지는 학생 실행 자산으로 사용하지 않았습니다.

## 구현

목표는 `주어진 방향과 거리만큼 점을 이동하기`, 성취기준은 `[4수03-04, 4수03-05]`입니다. 10개 문항은 네 방향의 한 번 이동, 이동 전후 설명, 두 방향의 연속 이동, 순서를 바꾸어 같은 도착점 찾기, 이동 전 점 거꾸로 찾기, 세 번의 보물 신호 이동을 다룹니다. 모눈·점·이동 신호는 구조화 데이터에서 만든 HTML·SVG로 표현합니다. 정답 판정은 출발점·후보점·도착점·명령 데이터가 담당하며 생성 그림은 관제탑 진행과 보상 분위기를 돕는 context-only 자산입니다. 좌표·음수·공식은 학생 화면에 도입하지 않았습니다.

## 자산·라인리지

17개 신규 자산(표지·제목·튜토리얼 2·보상 7·결과 6)을 `mathmon-grade4-official-pack-v1`의 `mathmon-grade4-01-sugar-glider` 하늘주머니몬 기준 이미지와 함께 실제 순차 imagegen 호출로 생성했습니다. 원본은 `_lessons/4-2-6-1-mathmon-point-move-control/assets/*-source.png`, runtime은 1280×800 `*-generated.webp`입니다. 실제 generation ID·프롬프트 SHA-256·원본/runtime SHA-256·참조 입력·허용 후처리 계보는 `_lessons/4-2-6-1-mathmon-point-move-control/assets/generation-lineage.json`에 기록했습니다. 픽셀 합성이나 HTML/CSS 글자 덮어쓰기는 사용하지 않았습니다. 전역 provider registry는 변경하지 않았고 현재 라인리지는 `honest-nonstrict-provider-lineage`만 주장합니다.

보상 7종은 `reward-states-contact-sheet.png`, 결과 6종은 `result-states-contact-sheet.png`에서 함께 확인할 수 있습니다. 결과 6장은 오른쪽 패널 위쪽에 `첫 신호`, `방향 등불`, `이동 지도`, `관제 책상`, `점 이동 관제탑`, `별빛 점 이동 관제탑`을 생성 래스터로 직접 넣고, 아래쪽 동적 정답 수·다음 목표 영역을 비운 서로 다른 관제탑 완성 장면입니다. 최종 1280×800 PNG에서 패널 중심을 단계별로 픽셀 측정해 계약과 lineage에 기록했습니다.

## 튜토리얼

튜토리얼은 정확히 2장입니다. 첫 포스터는 `출발점을 먼저 찾아요`, `위·아래·왼쪽·오른쪽을 확인해요`, `모눈 한 칸씩 정확히 세어요`로 한 번 이동하는 법을 보여 줍니다. 둘째 포스터는 `첫 이동을 끝낸 곳에서 다시 출발해요`, `방향이 바뀌면 화살표도 꺾어요`, `도착한 점과 알맞은 설명을 골라요`, `10문제를 풀면 하늘주머니몬의 관제탑이 6단계로 완성돼요`로 연속 이동과 보상 목표를 안내합니다. 두 포스터의 한국어 안내문과 이동 버튼은 생성 래스터 자체에 포함했고 HTML/CSS 글자 덮어쓰기를 사용하지 않았습니다.

## 학생 문구 검토

Humanizer 기준으로 학생 문구를 묶어 검토했습니다. `출발점에서 위쪽으로 모눈 세 칸을 세어요`, `첫 이동이 끝난 곳에서 두 번째 이동을 시작해요`, `도착점에서 이동 방향을 거꾸로 생각해요`처럼 4학년 학생이 소리 내어 읽어도 바로 이해할 수 있는 말로 확정했습니다. `방향 벡터`, `좌표`, `변환` 같은 앞선 개념어는 학생 문구에 쓰지 않고 교과서의 `위쪽·아래쪽·왼쪽·오른쪽·칸` 표현을 유지했습니다.

## 검증 결과

- `node scripts/test-mathmon-grade4-point-move-control-phase-a.mjs`: PASS
- `node scripts/build-lesson.mjs 4-2-6-1-mathmon-point-move-control`: PASS
...
- 실행본 SHA-256: `efaca127f1046e0f7d8f83f98192a2edeb454192cfc127ac574d4d154564b96a`
- 생성 시각: `2026-08-31T05:57:06.978Z`
- 등록 회귀 이름: `3개`
- 실제 실행 화면 조건: `3개`
- 동일 조건 별칭 통합: `0개`
...
- 학생이 보는 것: 점 이동 관제탑 제목과 한 줄 목표, 시작 버튼을 봅니다.
- 판단하거나 누르는 것: 게임을 시작할 준비가 되면 시작을 누릅니다.
- 화면에서 확인되는 수학 관계: 방향과 거리만큼 점 이동을 배우는 차시임을 확인합니다.
...
- 학생이 보는 것: 방향과 거리만큼 점 이동 문제를 푸는 방법과 게임 흐름을 그림으로 봅니다.
- 판단하거나 누르는 것: 그림 속 순서와 누를 곳을 확인한 뒤 다음 행동 버튼을 누릅니다.
- 화면에서 확인되는 수학 관계: 방향과 거리만큼 점 이동에서 무엇을 비교하거나 계산하는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 방향과 거리만큼 점 이동을 이용해 선택지를 판단합니다.
...
- 화면에서 확인되는 수학 관계: 방향과 거리만큼 점 이동의 관계와 고른 답이 왜 맞지 않는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 방향과 거리만큼 점 이동의 완성값을 보상 화면 전에 다시 확인합니다.
...
- 판단하거나 누르는 것: 이번 관제탑 단계 변화를 확인하기 위해 열기를 누릅니다.
...
- 학생이 보는 것: 보상 사건 그림과 이번 관제탑 단계 변화, 다음 행동 버튼을 봅니다.
...
- 화면에서 확인되는 수학 관계: 한 판의 정답과 관제탑 단계 변화가 하나의 결과 단계로 정리됩니다.
...
#### 결과 단계 · direction-light · `engine-flow-desktop-08a-result-direction-light.png`
...
#### 결과 단계 · route-board · `engine-flow-desktop-08a-result-route-board.png`
...
#### 결과 단계 · control-desk · `engine-flow-desktop-08a-result-control-desk.png`
...
#### 결과 단계 · point-control-tower · `engine-flow-desktop-08a-result-point-control-tower.png`
...
#### 결과 단계 · starlight-point-control-tower · `engine-flow-desktop-08a-result-starlight-point-control-tower.png`
...
#### 결과 단계 · direction-light · `engine-flow-tablet-landscape-08a-result-direction-light.png`
...
#### 결과 단계 · route-board · `engine-flow-tablet-landscape-08a-result-route-board.png`
...
#### 결과 단계 · control-desk · `engine-flow-tablet-landscape-08a-result-control-desk.png`
...
#### 결과 단계 · point-control-tower · `engine-flow-tablet-landscape-08a-result-point-control-tower.png`
...
#### 결과 단계 · starlight-point-control-tower · `engine-flow-tablet-landscape-08a-result-starlight-point-control-tower.png`
...
#### 결과 단계 · direction-light · `engine-flow-reported-design-feedback-1079x842-08a-result-direction-light.png`
...
#### 결과 단계 · route-board · `engine-flow-reported-design-feedback-1079x842-08a-result-route-board.png`
...
#### 결과 단계 · control-desk · `engine-flow-reported-design-feedback-1079x842-08a-result-control-desk.png`
...
#### 결과 단계 · point-control-tower · `engine-flow-reported-design-feedback-1079x842-08a-result-point-control-tower.png`
...
#### 결과 단계 · starlight-point-control-tower · `engine-flow-reported-design-feedback-1079x842-08a-result-starlight-point-control-tower.png`
