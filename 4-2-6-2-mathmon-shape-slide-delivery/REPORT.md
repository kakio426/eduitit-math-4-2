...
제공된 비상교육 4-2 교사용 교과서 PDF의 6단원 `평면도형의 이동` 가운데 도형 밀기 자료를 학습 근거로 사용했습니다. PDF 242–245쪽 ↔ 교과서 136–139쪽에서 도형을 위쪽·아래쪽·왼쪽·오른쪽으로 밀고, 밀기 전후에는 모양·크기·방향이 그대로이며 위치만 달라짐을 확인했습니다. PDF 246–247쪽 ↔ 수학 익힘 80–81쪽에서는 밀기 결과를 고르거나 그리고, 오른쪽 4칸과 위쪽 3칸을 차례를 바꾸어 미는 문제를 확인했습니다. 교과서 이미지는 학생 실행 자산으로 사용하지 않았습니다.

## 구현

목표는 `도형을 평행이동한 결과를 알아보기`, 성취기준은 `[4수03-04]`입니다. 10개 문항은 네 방향의 한 번 밀기, 이동 전후 설명, 밀기 전후에 그대로인 성질, 잘못 민 그림 찾기, 두 방향의 연속 밀기, 차례를 바꾸어 같은 위치 찾기, 처음 도형 거꾸로 찾기를 다룹니다. 모눈과 다각형은 구조화 데이터에서 만든 SVG로 표현합니다. 정답 판정은 꼭짓점과 이동 명령 데이터가 담당하며 생성 그림은 택배소 진행과 보상 분위기를 돕는 context-only 자산입니다.

## 자산·라인리지

17개 신규 자산(표지·제목·튜토리얼 2·보상 7·결과 6)을 `mathmon-grade4-official-pack-v1`의 `mathmon-grade4-02-bison` 들소몬 기준 이미지와 함께 실제 순차 imagegen 호출로 생성했습니다. 원본은 `_lessons/4-2-6-2-mathmon-shape-slide-delivery/assets/*-source.png`, runtime은 1280×800 `*-generated.webp`입니다. 실제 generation ID·프롬프트 SHA-256·원본/runtime SHA-256·참조 입력·허용 후처리 계보는 `_lessons/4-2-6-2-mathmon-shape-slide-delivery/assets/generation-lineage.json`에 기록했습니다. 픽셀 합성이나 HTML/CSS 글자 덮어쓰기는 사용하지 않았습니다. 전역 provider registry는 변경하지 않았고 현재 라인리지는 `honest-nonstrict-provider-lineage`만 주장합니다.

보상 7종은 `reward-states-contact-sheet.png`, 결과 6종은 `result-states-contact-sheet.png`에서 함께 확인할 수 있습니다. 결과 6장은 오른쪽 패널 위쪽에 `첫 상자`, `이동 레일`, `분류 선반`, `배송 작업대`, `도형 밀기 택배`, `별빛 도형 밀기 택배`를 생성 래스터로 직접 넣고, 아래쪽 동적 정답 수·다음 목표 영역을 비운 서로 다른 택배소 완성 장면입니다.

## 튜토리얼

튜토리얼은 정확히 2장입니다. 첫 포스터는 `모든 꼭짓점을 같은 방향으로 옮겨요`, `모양과 크기는 바뀌지 않아요`, `위치만 달라지는지 확인해요`로 한 번 미는 법을 보여 줍니다. 둘째 포스터는 `첫 이동이 끝난 곳에서 다시 시작해요`, `도형의 방향은 돌리지 않아요`, `민 후의 도형과 알맞은 설명을 골라요`, `10문제를 풀면 들소몬의 택배소가 6단계로 완성돼요`로 연속 밀기와 보상 목표를 안내합니다. 두 포스터의 한국어 안내문과 이동 버튼은 생성 래스터 자체에 포함했고 HTML/CSS 글자 덮어쓰기를 사용하지 않았습니다.

## 학생 문구 검토

Humanizer 기준으로 학생 문구를 묶어 검토했습니다. `세 꼭짓점을 모두 오른쪽으로 세 칸씩 옮겨요`, `첫 이동이 끝난 도형에서 두 번째 이동을 시작해요`, `민 뒤 도형에서 반대 방향으로 되짚어요`처럼 4학년 학생이 바로 이해할 수 있는 말로 확정했습니다. `벡터`, `좌표`, `변환` 같은 앞선 개념어는 학생 문구에 쓰지 않고 교과서의 `위쪽·아래쪽·왼쪽·오른쪽·칸·밀기` 표현을 유지했습니다.

## 검증 결과

- `node scripts/test-mathmon-grade4-shape-slide-delivery-phase-a.mjs`: PASS
- `node scripts/build-lesson.mjs 4-2-6-2-mathmon-shape-slide-delivery`: PASS
...
- 실행본 SHA-256: `2f974322c3f5a891c53bf4446907e3012348ed82d6f65b016ca326c39ef2ea6d`
- 생성 시각: `2026-08-31T05:57:28.678Z`
- 등록 회귀 이름: `3개`
- 실제 실행 화면 조건: `3개`
- 동일 조건 별칭 통합: `0개`
...
- 학생이 보는 것: 도형 밀기 택배 제목과 한 줄 목표, 시작 버튼을 봅니다.
- 판단하거나 누르는 것: 게임을 시작할 준비가 되면 시작을 누릅니다.
- 화면에서 확인되는 수학 관계: 평면도형 밀기을 배우는 차시임을 확인합니다.
...
- 학생이 보는 것: 평면도형 밀기 문제를 푸는 방법과 게임 흐름을 그림으로 봅니다.
- 판단하거나 누르는 것: 그림 속 순서와 누를 곳을 확인한 뒤 다음 행동 버튼을 누릅니다.
- 화면에서 확인되는 수학 관계: 평면도형 밀기에서 무엇을 비교하거나 계산하는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 평면도형 밀기을 이용해 선택지를 판단합니다.
...
- 화면에서 확인되는 수학 관계: 평면도형 밀기의 관계와 고른 답이 왜 맞지 않는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 평면도형 밀기의 완성값을 보상 화면 전에 다시 확인합니다.
...
- 판단하거나 누르는 것: 이번 택배소 단계 변화를 확인하기 위해 열기를 누릅니다.
...
- 학생이 보는 것: 보상 사건 그림과 이번 택배소 단계 변화, 다음 행동 버튼을 봅니다.
...
- 화면에서 확인되는 수학 관계: 한 판의 정답과 택배소 단계 변화가 하나의 결과 단계로 정리됩니다.
- 다음 상태로 넘어가는 이유: 다시를 누르면 새 문제 순서와 새 보상 흐름으로 시작합니다.

#### 결과 단계 · first-box · `engine-flow-desktop-08a-result-first-box.png`
...
#### 결과 단계 · moving-rail · `engine-flow-desktop-08a-result-moving-rail.png`
...
#### 결과 단계 · sorting-shelf · `engine-flow-desktop-08a-result-sorting-shelf.png`
...
#### 결과 단계 · delivery-workbench · `engine-flow-desktop-08a-result-delivery-workbench.png`
...
#### 결과 단계 · shape-slide-delivery · `engine-flow-desktop-08a-result-shape-slide-delivery.png`
...
#### 결과 단계 · starlight-shape-delivery · `engine-flow-desktop-08a-result-starlight-shape-delivery.png`
...
#### 결과 단계 · first-box · `engine-flow-tablet-landscape-08a-result-first-box.png`
...
#### 결과 단계 · moving-rail · `engine-flow-tablet-landscape-08a-result-moving-rail.png`
...
#### 결과 단계 · sorting-shelf · `engine-flow-tablet-landscape-08a-result-sorting-shelf.png`
...
#### 결과 단계 · delivery-workbench · `engine-flow-tablet-landscape-08a-result-delivery-workbench.png`
...
#### 결과 단계 · shape-slide-delivery · `engine-flow-tablet-landscape-08a-result-shape-slide-delivery.png`
...
#### 결과 단계 · starlight-shape-delivery · `engine-flow-tablet-landscape-08a-result-starlight-shape-delivery.png`
...
#### 결과 단계 · first-box · `engine-flow-reported-design-feedback-1079x842-08a-result-first-box.png`
...
#### 결과 단계 · moving-rail · `engine-flow-reported-design-feedback-1079x842-08a-result-moving-rail.png`
...
#### 결과 단계 · sorting-shelf · `engine-flow-reported-design-feedback-1079x842-08a-result-sorting-shelf.png`
...
#### 결과 단계 · delivery-workbench · `engine-flow-reported-design-feedback-1079x842-08a-result-delivery-workbench.png`
...
#### 결과 단계 · shape-slide-delivery · `engine-flow-reported-design-feedback-1079x842-08a-result-shape-slide-delivery.png`
...
#### 결과 단계 · starlight-shape-delivery · `engine-flow-reported-design-feedback-1079x842-08a-result-starlight-shape-delivery.png`
