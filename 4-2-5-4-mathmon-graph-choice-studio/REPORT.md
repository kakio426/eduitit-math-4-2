...
제공된 비상교육 4-2 교사용 교과서 PDF의 5단원 `꺾은선그래프` 자료를 학습 근거로 사용했습니다. PDF 218–219쪽 ↔ 교과서 122–123쪽에서 항목별 수량을 한눈에 비교할 때는 막대그래프, 시간에 따른 변화를 살필 때는 꺾은선그래프가 편리함을 확인했습니다. 같은 연도별 자료도 `값 비교`와 `변화 관찰`처럼 목적에 따라 서로 다른 그래프로 나타낼 수 있음을 살폈습니다. PDF 220–221쪽 ↔ 수학 익힘 74–75쪽에서는 지역별 도서관 수, 월별 몸무게, 연도별 메달 수에 알맞은 그래프를 고르고, 세로 눈금에 따라 모양이 달라 보여도 실제 수와 눈금을 읽어야 함을 확인했습니다. 교과서 이미지는 학생 실행 자산으로 사용하지 않았습니다.

## 구현

목표는 `자료의 성격에 따라 막대그래프와 꺾은선그래프 중 알맞은 그래프를 선택하고 근거를 말하기`, 성취기준은 `[4수04-02, 4수04-03]`입니다. 10개 문항은 항목별 수량 비교, 시간에 따른 변화, 같은 자료의 서로 다른 목적, 두 그래프의 편리한 점, 시간 순서가 아닌 자료, 실제 수와 눈금 읽기를 다룹니다. 자료는 읽기 쉬운 HTML 표로, 그래프 미리보기는 수치 데이터에서 만든 SVG로 표현합니다. 정답 판정은 구조화된 JS 데이터가 담당하며 생성 그림은 방송국 진행과 보상 분위기를 돕는 context-only 자산입니다.

## 자산·라인리지

17개 신규 자산(표지·제목·튜토리얼 2·보상 7·결과 6)을 `mathmon-grade4-official-pack-v1`의 `mathmon-grade4-10-okapi` 줄무늬오카피몬 기준 이미지와 함께 실제 순차 imagegen 호출로 생성했습니다. 원본은 `_lessons/4-2-5-4-mathmon-graph-choice-studio/assets/*-source.png`, runtime은 1280×800 `*-generated.webp`입니다. 실제 generation ID·프롬프트 SHA-256·원본/runtime SHA-256·참조 입력·허용 후처리 계보는 `_lessons/4-2-5-4-mathmon-graph-choice-studio/assets/generation-lineage.json`에 기록했습니다. 픽셀 합성이나 HTML/CSS 글자 덮어쓰기는 사용하지 않았습니다. 전역 provider registry는 변경하지 않았고 현재 라인리지는 `honest-nonstrict-provider-lineage`만 주장합니다.

보상 7종은 `reward-states-contact-sheet.png`, 결과 6종은 `result-states-contact-sheet.png`에서 함께 확인할 수 있습니다. 결과 6장은 오른쪽 패널 위쪽에 `첫 자료`, `비교 화면`, `변화 화면`, `선택 편집실`, `그래프 방송국`, `별빛 그래프 방송국`을 생성 래스터로 직접 넣고, 아래쪽 동적 정답 수·다음 목표 영역을 비운 서로 다른 방송국 완성 장면입니다. 최종 1280×800 PNG에서 패널 중심을 단계별로 픽셀 측정해 계약과 lineage에 기록했습니다.

## 튜토리얼

튜토리얼은 정확히 2장입니다. 첫 포스터는 `서로 다른 항목의 수량을 나란히 비교해요`, `막대의 길이로 크고 작음을 살펴봐요`, `자료의 목적을 먼저 확인해요`로 막대그래프를 고르는 기준을 보여 줍니다. 둘째 포스터는 `시간 순서대로 값을 살펴봐요`, `이어진 선분으로 증가와 감소를 읽어요`, `그래프 이름과 알맞은 근거를 함께 골라요`, `10문제를 풀면 줄무늬오카피몬의 방송국이 6단계로 완성돼요`로 꺾은선그래프와 보상 목표를 안내합니다. 두 포스터의 한국어 안내문과 이동 버튼은 생성 래스터 자체에 포함했고 HTML/CSS 글자 덮어쓰기를 사용하지 않았습니다.

## 학생 문구 검토

Humanizer 기준으로 학생 문구를 묶어 검토했습니다. `여러 지역의 도서관 수를 한눈에 비교하기`, `시간이 지나며 높이가 어떻게 변했는지 알아보기`, `모양만 보지 않고 실제 수와 세로 눈금을 읽어야 해요`처럼 4학년 학생이 소리 내어 읽어도 바로 이해할 수 있는 말로 확정했습니다. 그래프 이름만 외우게 하지 않고 자료의 항목과 목적을 함께 말하도록 했으며 기울기나 변화율 공식을 덧붙이지 않았습니다.

## 검증 결과

- `node scripts/test-mathmon-grade4-graph-choice-studio-phase-a.mjs`: PASS
- `node scripts/build-lesson.mjs 4-2-5-4-mathmon-graph-choice-studio`: PASS
...
- 실행본 SHA-256: `affa5eabe8c5f6548f22badaf7d03b45f0e06a82875737bc166c7a97a50c2241`
- 생성 시각: `2026-08-31T05:56:47.688Z`
- 등록 회귀 이름: `3개`
- 실제 실행 화면 조건: `3개`
- 동일 조건 별칭 통합: `0개`
...
- 학생이 보는 것: 그래프 선택 방송국 제목과 한 줄 목표, 시작 버튼을 봅니다.
- 판단하거나 누르는 것: 게임을 시작할 준비가 되면 시작을 누릅니다.
- 화면에서 확인되는 수학 관계: 자료의 성격과 목적에 알맞은 그래프 선택을 배우는 차시임을 확인합니다.
...
- 학생이 보는 것: 자료의 성격과 목적에 알맞은 그래프 선택 문제를 푸는 방법과 게임 흐름을 그림으로 봅니다.
- 판단하거나 누르는 것: 그림 속 순서와 누를 곳을 확인한 뒤 다음 행동 버튼을 누릅니다.
- 화면에서 확인되는 수학 관계: 자료의 성격과 목적에 알맞은 그래프 선택에서 무엇을 비교하거나 계산하는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 자료의 성격과 목적에 알맞은 그래프 선택을 이용해 선택지를 판단합니다.
...
- 화면에서 확인되는 수학 관계: 자료의 성격과 목적에 알맞은 그래프 선택의 관계와 고른 답이 왜 맞지 않는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 자료의 성격과 목적에 알맞은 그래프 선택의 완성값을 보상 화면 전에 다시 확인합니다.
...
- 판단하거나 누르는 것: 이번 방송국 단계 변화를 확인하기 위해 열기를 누릅니다.
...
- 학생이 보는 것: 보상 사건 그림과 이번 방송국 단계 변화, 다음 행동 버튼을 봅니다.
...
- 화면에서 확인되는 수학 관계: 한 판의 정답과 방송국 단계 변화가 하나의 결과 단계로 정리됩니다.
- 다음 상태로 넘어가는 이유: 다시를 누르면 새 문제 순서와 새 보상 흐름으로 시작합니다.

#### 결과 단계 · first-card · `engine-flow-desktop-08a-result-first-card.png`
...
#### 결과 단계 · comparison-screen · `engine-flow-desktop-08a-result-comparison-screen.png`
...
#### 결과 단계 · change-screen · `engine-flow-desktop-08a-result-change-screen.png`
...
#### 결과 단계 · editing-desk · `engine-flow-desktop-08a-result-editing-desk.png`
...
#### 결과 단계 · graph-station · `engine-flow-desktop-08a-result-graph-station.png`
...
#### 결과 단계 · starlight-graph-station · `engine-flow-desktop-08a-result-starlight-graph-station.png`
...
#### 결과 단계 · first-card · `engine-flow-tablet-landscape-08a-result-first-card.png`
...
#### 결과 단계 · comparison-screen · `engine-flow-tablet-landscape-08a-result-comparison-screen.png`
...
#### 결과 단계 · change-screen · `engine-flow-tablet-landscape-08a-result-change-screen.png`
...
#### 결과 단계 · editing-desk · `engine-flow-tablet-landscape-08a-result-editing-desk.png`
...
#### 결과 단계 · graph-station · `engine-flow-tablet-landscape-08a-result-graph-station.png`
...
#### 결과 단계 · starlight-graph-station · `engine-flow-tablet-landscape-08a-result-starlight-graph-station.png`
...
#### 결과 단계 · first-card · `engine-flow-reported-design-feedback-1079x842-08a-result-first-card.png`
...
#### 결과 단계 · comparison-screen · `engine-flow-reported-design-feedback-1079x842-08a-result-comparison-screen.png`
...
#### 결과 단계 · change-screen · `engine-flow-reported-design-feedback-1079x842-08a-result-change-screen.png`
...
#### 결과 단계 · editing-desk · `engine-flow-reported-design-feedback-1079x842-08a-result-editing-desk.png`
...
#### 결과 단계 · graph-station · `engine-flow-reported-design-feedback-1079x842-08a-result-graph-station.png`
...
#### 결과 단계 · starlight-graph-station · `engine-flow-reported-design-feedback-1079x842-08a-result-starlight-graph-station.png`
