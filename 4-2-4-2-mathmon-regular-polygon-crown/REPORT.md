...
제공된 비상교육 4-2 교사용 교과서 PDF의 4단원 `다각형` 자료를 학습 근거로 사용했습니다. PDF 164–165쪽 ↔ 교과서 90–91쪽에서 변의 길이가 모두 같고 각의 크기가 모두 같은 다각형이라는 정다각형의 뜻, 정삼각형·정사각형·정오각형·정육각형의 이름을 확인했습니다. PDF 166–167쪽 ↔ 수학 익힘 56–57쪽에서 정다각형 찾기, 정구각형 이름 붙이기, 둘레가 21 cm인 정칠각형의 한 변 길이를 구하는 활동을 확인했습니다. 교과서 이미지는 학생 실행 자산으로 사용하지 않았습니다.

## 구현

목표는 `정다각형의 성질을 알고 정다각형을 찾기`, 성취기준은 `[4수03-11]`입니다. 10개 문항은 정다각형의 뜻, 변의 길이만 같은 반례, 각의 크기만 같은 반례, 정삼각형·정사각형·정오각형·정육각형, 기울어진 정오각형, 정구각형의 이름, 정칠각형의 둘레와 한 변 길이 관계를 다룹니다. 도형과 측정값은 구조화 데이터와 읽기 쉬운 HTML/CSS로 표현하며 정답 판정은 JS 데이터가 담당합니다. 생성 그림은 왕관 공방과 진행 분위기를 돕는 context-only 자산이며 정답의 유일한 근거가 아닙니다.

## 자산·라인리지

17개 신규 자산(표지·제목·튜토리얼 2·보상 7·결과 6)을 `mathmon-grade4-official-pack-v1`의 `mathmon-grade4-04-kingfisher` 물총새몬 기준 이미지와 함께 실제 순차 imagegen 호출로 생성했습니다. 원본은 `_lessons/4-2-4-2-mathmon-regular-polygon-crown/assets/*-source.png`, runtime은 1280×800 `*-generated.webp`입니다. 실제 생성 ID·프롬프트 SHA-256·원본/runtime SHA-256·허용 후처리 계보는 `_lessons/4-2-4-2-mathmon-regular-polygon-crown/assets/generation-lineage.json`에 기록했습니다. 픽셀 합성이나 HTML/CSS 글자 덮어쓰기는 사용하지 않았습니다. 전역 provider registry는 변경하지 않았고 현재 라인리지는 `honest-nonstrict-provider-lineage`만 주장합니다.

보상 7종은 `reward-states-contact-sheet.png`, 결과 6종은 `result-states-contact-sheet.png`에서 함께 확인할 수 있습니다. 결과 6장은 오른쪽 패널 위쪽에 각 단계 이름을 생성 래스터로 직접 넣고, 아래쪽 동적 정답 수·다음 목표 영역을 비운 서로 다른 왕관 완성 장면입니다.

## 튜토리얼

튜토리얼은 정확히 2장입니다. 첫 포스터는 `변의 길이가 모두 같고, 각의 크기도 모두 같아야 정다각형이에요`와 `둘 중 하나만 같으면 정다각형이 아니에요`로 두 조건을 보여 줍니다. 둘째 포스터는 `변이 5개면 정오각형, 6개면 정육각형이에요`와 `10문제를 풀면 물총새몬의 왕관이 6단계로 완성돼요`로 이름 붙이기와 보상 목표를 보여 줍니다. 두 포스터의 한국어 안내문과 이동 버튼은 생성 래스터 자체에 포함했고, HTML/CSS 글자 덮어쓰기를 사용하지 않았습니다.

## 학생 문구 검토

Humanizer 기준으로 학생 문구를 묶어 검토했습니다. `두 조건을 함께 봐요`, `긴 변과 짧은 변이 있어요`, `놓인 방향보다 변과 각의 조건을 살펴봐요`처럼 4학년 학생이 소리 내어 읽어도 바로 이해할 수 있는 말로 확정했습니다. 정칠각형의 계산도 `21÷7=3이므로 한 변의 길이는 3 cm예요.`처럼 실제 판단 순서대로 풀어 썼습니다.

## 검증 결과

- `node scripts/test-mathmon-grade4-regular-polygon-crown-phase-a.mjs`: PASS
- `node scripts/build-lesson.mjs 4-2-4-2-mathmon-regular-polygon-crown`: PASS
...
- 실행본 SHA-256: `a1f4bb09652273d20a63d0356e8428e36a4e6358f3c8bc7fc0435305cb3796d2`
- 생성 시각: `2026-08-31T05:54:49.323Z`
- 등록 회귀 이름: `3개`
- 실제 실행 화면 조건: `3개`
- 동일 조건 별칭 통합: `0개`
...
- 학생이 보는 것: 정다각형 왕관 제목과 한 줄 목표, 시작 버튼을 봅니다.
- 판단하거나 누르는 것: 게임을 시작할 준비가 되면 시작을 누릅니다.
- 화면에서 확인되는 수학 관계: 정다각형의 뜻과 이름을 배우는 차시임을 확인합니다.
...
- 학생이 보는 것: 정다각형의 뜻과 이름 문제를 푸는 방법과 게임 흐름을 그림으로 봅니다.
- 판단하거나 누르는 것: 그림 속 순서와 누를 곳을 확인한 뒤 다음 행동 버튼을 누릅니다.
- 화면에서 확인되는 수학 관계: 정다각형의 뜻과 이름에서 무엇을 비교하거나 계산하는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 정다각형의 뜻과 이름을 이용해 선택지를 판단합니다.
...
- 화면에서 확인되는 수학 관계: 정다각형의 뜻과 이름의 관계와 고른 답이 왜 맞지 않는지 확인합니다.
...
- 화면에서 확인되는 수학 관계: 정다각형의 뜻과 이름의 완성값을 보상 화면 전에 다시 확인합니다.
...
- 판단하거나 누르는 것: 이번 왕관 단계 변화를 확인하기 위해 열기를 누릅니다.
...
- 학생이 보는 것: 보상 사건 그림과 이번 왕관 단계 변화, 다음 행동 버튼을 봅니다.
...
- 화면에서 확인되는 수학 관계: 한 판의 정답과 왕관 단계 변화가 하나의 결과 단계로 정리됩니다.
- 다음 상태로 넘어가는 이유: 다시를 누르면 새 문제 순서와 새 보상 흐름으로 시작합니다.

#### 결과 단계 · crown-frame · `engine-flow-desktop-08a-result-crown-frame.png`
...
#### 결과 단계 · equal-side-gem · `engine-flow-desktop-08a-result-equal-side-gem.png`
...
#### 결과 단계 · equal-angle-light · `engine-flow-desktop-08a-result-equal-angle-light.png`
...
#### 결과 단계 · regular-crown · `engine-flow-desktop-08a-result-regular-crown.png`
...
#### 결과 단계 · royal-vault · `engine-flow-desktop-08a-result-royal-vault.png`
...
#### 결과 단계 · rainbow-palace · `engine-flow-desktop-08a-result-rainbow-palace.png`
...
#### 결과 단계 · crown-frame · `engine-flow-tablet-landscape-08a-result-crown-frame.png`
...
#### 결과 단계 · equal-side-gem · `engine-flow-tablet-landscape-08a-result-equal-side-gem.png`
...
#### 결과 단계 · equal-angle-light · `engine-flow-tablet-landscape-08a-result-equal-angle-light.png`
...
#### 결과 단계 · regular-crown · `engine-flow-tablet-landscape-08a-result-regular-crown.png`
...
#### 결과 단계 · royal-vault · `engine-flow-tablet-landscape-08a-result-royal-vault.png`
...
#### 결과 단계 · rainbow-palace · `engine-flow-tablet-landscape-08a-result-rainbow-palace.png`
...
#### 결과 단계 · crown-frame · `engine-flow-reported-design-feedback-1079x842-08a-result-crown-frame.png`
...
#### 결과 단계 · equal-side-gem · `engine-flow-reported-design-feedback-1079x842-08a-result-equal-side-gem.png`
...
#### 결과 단계 · equal-angle-light · `engine-flow-reported-design-feedback-1079x842-08a-result-equal-angle-light.png`
...
#### 결과 단계 · regular-crown · `engine-flow-reported-design-feedback-1079x842-08a-result-regular-crown.png`
...
#### 결과 단계 · royal-vault · `engine-flow-reported-design-feedback-1079x842-08a-result-royal-vault.png`
...
#### 결과 단계 · rainbow-palace · `engine-flow-reported-design-feedback-1079x842-08a-result-rainbow-palace.png`
